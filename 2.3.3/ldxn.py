...
try:
    if platform.system() == "Windows":
        ...
        dllpath = str(Path(__file__).parent) + str(Path('/', 'ControlCAN-ldxn.dll'))
        CANalystII = CDLL(dllpath)
    else:
        CANalystII = CDLL("./libusbcan.so") # will never work for LDXN
        CANalystII = None
        logger.info("LDXN do not has linux driver yet")
